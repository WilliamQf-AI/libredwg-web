import createModule from '../lib/libredwg-web.js';
import { LibreDwgConverter } from './converter/index.js';
import { dwgCodePageToEncoding, dwgVersions } from './database/index.js';
import { SvgConverter } from './svg/index.js';
import { Dwg_File_Type, Dwg_Object_Type } from './types/index.js';
export { createModule, Dwg_File_Type, Dwg_Object_Type };
// Provide a default export for backward compatibility with code that uses default import
...
export class LibreDwg {
    ...
    dwg_read_data(fileContent, fileType) {
        if (fileType == Dwg_File_Type.DWG) {
            ...
            // DXF support may not be present in the current WASM build.
            if (typeof this.wasmInstance.dxf_read_file !== 'function') {
                throw new Error('DXF support is not available in the loaded WASM module (dxf_read_file missing). Please rebuild libredwg with DXF support, or use a JS DXF parser fallback.');
            }
    ...
    dwg_ref_get_object_name(ref_ptr) {
        ...
        try {
            ...
            if (!obj_tio)
                return '';
            const nameVal = this.dwg_dynapi_entity_value(obj_tio, 'name');
            if (!nameVal)
                return '';
            return nameVal.data;
        }
        catch (err) {
            // Fail safe: log and return empty string instead of throwing
            if (this.wasmInstance && this.wasmInstance.printErr)
                this.wasmInstance.printErr && this.wasmInstance.printErr('dwg_ref_get_object_name error: ' + err);
            return '';
        }
    }
    /**
    ...
    /**
     * 将 Dwg_Data 保存为指定版本的 Uint8Array
     * @param {number} dataPtr - Dwg_Data 的指针
     * @param {number} versionEnum - 目标版本的枚举值 (例如 R_2000 是 13)
     * @returns {Uint8Array} - 返回文件二进制数据
     */
    dwg_write_data(dataPtr, versionEnum) {
        const wasm = this.wasmInstance;
        const tmpFile = 'output.dwg';

        try {
            // 1. 设置目标版本
            if (typeof wasm.dwg_set_version_type === 'function') {
                wasm.dwg_set_version_type(dataPtr, versionEnum);
            } else {
                console.warn("dwg_set_version_type not exported");
            }

            // 2. 调用写入函数 (写入 WASM 虚拟文件系统)
            const result = wasm.dwg_write_file(tmpFile, dataPtr);
            if (result !== 0) {
                throw new Error('Write DWG failed with error code: ' + result);
            }

            // 3. 从虚拟文件系统读取二进制数据
            const fileData = wasm.FS.readFile(tmpFile);
            
            // 4. 清理虚拟文件
            wasm.FS.unlink(tmpFile);

            return fileData; // 这是一个 Uint8Array
        } catch (e) {
    ...
    /**
     * 将 Dwg_Data 转换为 DXF 格式
     * @param {number} dataPtr - Dwg_Data 指针
     * @param {string} filename - 临时文件名
     * @param {boolean} binary - 是否二进制
     * @param {number} [targetVersion] - 目标版本枚举值 (建议传 13，即 R_2000)
     */
    dwg_to_dxf(dataPtr, filename, binary=false, targetVersion=13) { // 默认 R_2000
        const wasm = this.wasmInstance;
        
        if (typeof wasm.dxf_write_file !== 'function') {
            console.error("WASM module does not export 'dxf_write_file'.");
            return null;
        }
        ...
        try {
            // --- 新增：强制设置版本 ---
            if (targetVersion) {
                // LibreDwg 中 R_2000 的枚举值通常是 13
                // R_14 = 12, R_2000 = 13, R_2004 = 14, R_2007 = 15
                // 强制降级可以解决大部分 "Block not found" 问题
                if (typeof wasm.dwg_set_version_type === 'function') {
                    wasm.dwg_set_version_type(dataPtr, targetVersion);
                } else {
                    console.warn("dwg_set_version_type not exported");
                }
            }
            ...
            const result = binary 
                ? wasm.dxf_write_binary(filename, dataPtr) 
                : wasm.dxf_write_file(filename, dataPtr);
            
            if (result !== 0 && result >= 0x8000) {
                 throw new Error('Write DXF failed code: ' + result);
            }

            const fileData = wasm.FS.readFile(filename);
            wasm.FS.unlink(filename);
            return fileData;

        } catch (e) {
            console.error("DXF Conversion Error:", e);
            try { wasm.FS.unlink(filename); } catch(z){}
            return null;
        }
    }
}

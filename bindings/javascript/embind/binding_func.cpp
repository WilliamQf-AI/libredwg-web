...
emscripten::val dxf_read_file_wrapper(const std::string& filename) {
  ...
  emscripten::val result = emscripten::val::object();
  // If parser reported INVALIDDWG but produced objects, treat it as a
  // non-fatal warning so frontend can still inspect the parsed data.
  if (error == DWG_ERR_INVALIDDWG && dwg && dwg->num_objects) {
    result.set("error", 0);
    result.set("warning", error);
  } else {
    result.set("error", error);
  }
  result.set("data", reinterpret_cast<uintptr_t>(dwg));
...
// 添加设置版本的包装函数
void dwg_set_version_type_wrapper(uintptr_t dwg_ptr, int version_enum) {
  Dwg_Data* dwg = reinterpret_cast<Dwg_Data*>(dwg_ptr);
  if (dwg) {
    dwg->header.version = (Dwg_Version_Type)version_enum;
  }
}
 ...
 */
unsigned int dwg_get_num_classes_wrapper(uintptr_t dwg_ptr) {
  Dwg_Data* dwg = reinterpret_cast<Dwg_Data*>(dwg_ptr);
  return dwg_get_num_classes(dwg);
}
 ...
 */
emscripten::val dwg_get_class_wrapper(uintptr_t dwg_ptr, unsigned int index) {
  Dwg_Data* dwg = reinterpret_cast<Dwg_Data*>(dwg_ptr);
  dwg_class * cls = dwg_get_class(dwg, index);
  if (cls) {
    emscripten::val result = emscripten::val::object();
    result.set("num", cls->number);
    result.set("proxyflag", cls->proxyflag);
    result.set("appname", std::string(cls->appname));
    result.set("cppname", std::string(cls->cppname));
    result.set("dxfname", std::string(cls->dxfname));
    result.set("is_zombie", cls->is_zombie);
    result.set("item_class_id", cls->item_class_id);
    result.set("num_instances", cls->num_instances);
    result.set("dwg_version", cls->dwg_version);
    result.set("maint_version", cls->maint_version);
    return result;
  }
  return emscripten::val::null();
}
...
// 包装 dwg_write_dxf 函数，使其像 dwg_write_file 一样易用
int dxf_write_file_wrapper(const std::string& filename, uintptr_t dwg_ptr) {
  Dwg_Data* dwg = reinterpret_cast<Dwg_Data*>(dwg_ptr);
  
  // 初始化 Bit_Chain 结构 (参考 dwg2dxf.c)
  Bit_Chain dat = { 0 };
  dat.version = dwg->header.version;
  dat.from_version = dwg->header.from_version;
  
  // 打开文件 (在 WASM 中这是 MEMFS 的虚拟文件)
  dat.fh = fopen(filename.c_str(), "wb");
  if (!dat.fh) {
...
// 包装 dwg_write_dxf 函数，使其像 dwg_write_file 一样易用
int dxf_write_binary_wrapper(const std::string& filename, uintptr_t dwg_ptr) {
  Dwg_Data* dwg = reinterpret_cast<Dwg_Data*>(dwg_ptr);
...
EMSCRIPTEN_BINDINGS(libredwg_api) {
  ...
  //function("dwg_write_file", &dwg_write_file_wrapper);

  DEFINE_FUNC(dwg_get_num_classes);
  DEFINE_FUNC(dwg_get_class);
  ...
  function("dwg_set_version_type", &dwg_set_version_type_wrapper);

  function("dxf_write_file", &dxf_write_file_wrapper);
  function("dxf_write_binary", &dxf_write_binary_wrapper);
}

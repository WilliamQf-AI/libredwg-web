...
export class LibreDwg {
  ...
  /**
   * Write a Dwg_Data pointed to by `data` into a DWG file in the wasm FS.
   * Returns 0 on success or an error code.
   */
  dwg_write_file(filename: string, dataPtr: Dwg_Data_Ptr): number {
    // call the embind-exposed wrapper
    return (this.wasmInstance as any).dwg_write_file(filename, dataPtr);
  }

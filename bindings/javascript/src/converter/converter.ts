...
export class LibreDwgConverter {
  ...
  convert(data: Dwg_Data_Ptr) {
    ...
    for (let i = 0; i < num_objects; i++) {
      ...
      // Fallback: if model-space block was detected but resulted in no entities,
      // try to pick the first non-empty BLOCK_RECORD so the converter still
      // produces renderable `db.entities` (helps with some DXF variants).
      if ((!db.entities || db.entities.length === 0) && db.tables.BLOCK_RECORD.entries.length > 0) {
        for (const btr of db.tables.BLOCK_RECORD.entries) {
          if (Array.isArray(btr.entities) && btr.entities.length > 0) {
            db.entities = btr.entities
            console.warn('[LibreDwgConverter] model-space empty, falling back to block:', btr.name)
            break
          }
        }
      }

...
dwg_read_dxf (Bit_Chain *restrict dat, Dwg_Data *restrict dwg)
{
  ...
  /* Diagnostic summary: list BLOCK_HEADER records and basic entity counts.
   * This helps debug DXF files that set a model-space block but leave it
   * without entities (causing empty conversion results in JS).
   * This is non-invasive and only logs information.
   */
  {
    unsigned int bi;
    LOG_INFO("DXF: total_objects=%u, scanning BLOCK_HEADER records...", (unsigned)dwg->num_objects);
    for (bi = 0; bi < (unsigned)dwg->num_objects; bi++) {
      Dwg_Object *o = &dwg->object[bi];
      if (!o)
        continue;
      if (o->fixedtype == DWG_TYPE_BLOCK_HEADER && o->tio.object) {
        Dwg_Object_BLOCK_HEADER *hdr = o->tio.object->tio.BLOCK_HEADER;
        if (!hdr)
          continue;
        LOG_INFO("BTR: name=%s num_owned=%u block_entity=%s first_entity=%s last_entity=%s",
                 hdr->name ? hdr->name : "(null)",
                 (unsigned)hdr->num_owned,
                 hdr->block_entity ? "yes" : "no",
                 hdr->first_entity ? "yes" : "no",
                 hdr->last_entity ? "yes" : "no");
      }
    }
  }
